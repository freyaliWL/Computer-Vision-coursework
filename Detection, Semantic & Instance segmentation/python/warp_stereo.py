...
def mcbb(s1, s2, H1, H2):
    ...
    minx = int(np.floor(min(corners_x[0, :])))
    maxx = int(np.ceil(max(corners_x[0, :])))
    miny = int(np.floor(min(corners_x[1, :])))
    maxy = int(np.ceil(max(corners_x[1, :])))
    bb1 = [minx, miny, maxx, maxy]

    corners = np.array([[0, 0, s2[1], s2[1]], [0, s2[0], 0, s2[0]]])
    corners_x = p2t(H2, corners)

    # Use Python's built-in int
    minx = int(np.floor(min(corners_x[0, :])))
    maxx = int(np.ceil(max(corners_x[0, :])))
    miny = int(np.floor(min(corners_x[1, :])))
    maxy = int(np.ceil(max(corners_x[1, :])))
